...
export default function SubSync() {
    ...
    const selectedModelKey = config && model
        ...
        : null
    const requiredModels = [selectedModelKey, 'forced_aligner']
    ...
    const handleSubmit = async () => {
        ...
        if (modelsBlocked) {
            setError('必要的 ASR 模型尚未就緒，請先透過 Manager 完成下載')
            return
        }
        ...
        try {
            ...
            if (inputType === 'youtube') {
                ...
                resp = await fetchWithAuth('/api/youtube/analyze/upload', {
                    method: 'POST',
                    body: formData,
                })
            }
    ...
    const handleSaveSentences = async () => {
        if (!taskId) return
        try {
            const resp = await fetchWithAuth(`/api/tasks/${taskId}/sentences`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
    ...
    const handleRevertPunctuation = async () => {
        ...
        try {
            await fetchWithAuth(`/api/tasks/${taskId}/sentences`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
    ...
    return (
        <div className="fade-in">
            ...
            <ModelRequirement modelKeys={requiredModels} title="SubSync 所需 ASR 模型尚未就緒" />
                ...
                <div className="card subsync-input-card fade-in">
                    ...
                    <div className="subsync-url-row">
                        ...
                        <button
                            ...
                            disabled={modelsBlocked || (inputType === 'youtube' ? !url.trim() : !file)}
                            title={modelsBlocked ? '必要的 ASR 模型尚未下載' : undefined}
                        >
                            🚀 開始分析
                        </button>
                    </div>
